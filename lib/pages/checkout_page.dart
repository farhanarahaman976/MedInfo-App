...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  double get _totalPrice =>
      widget.cartItems.fold(0, (sum, item) => sum + item.displayPrice);
  ...
  Future<void> _placeOrder() async {
    ...
    try {
      final order = MedicineOrder(
        ...
        items: widget.cartItems.map((m) => OrderItem.fromMedicine(m)).toList(),
      ...
      Navigator.pushReplacement(
        ...
        MaterialPageRoute(
          builder: (_) =>
              OrderSuccessPage(orderId: orderId, totalAmount: _totalPrice),
        ),
      );
    } catch (e) {
      if (!mounted) return;
